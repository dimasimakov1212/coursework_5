...
[salary_higher]
query1=SELECT vacancy_name, salary_from, salary_to FROM vacancies WHERE (salary_from + salary_to) / 2 > (SELECT AVG((salary_from + salary_to) / 2) FROM vacancies WHERE salary_from <> 0 AND salary_to <> 0)
